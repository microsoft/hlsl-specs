...
Template type can be any type that totals or can be padded to 16 bytes.
...
\p Any declaration in the \texttt{cbuffer} declaration group that is not a
\textit{namespace-declaration} or a \textit{variable-declaration} is going to be
processed as if it was declared outside of the group at the
global scope.

\p Nesting of \texttt{cbuffer} declaration groups is not allowed.

\p Any \textit{namespace-declaration} in the \texttt{cbuffer} declaration group can contain only declarations allowed by \textit{cbuffer-declaration}.
...
\p Vectors are aligned by the size of a single vector element type.
...
This function returns true is the value was accessed from a fully committed
