...
The operation flags are generated by OR'ing together values from these constants:

```
const uint8_t kMatVecMulFlag_UnsignedInputVector = 0x1;
const uint8_t kMatVelMulFlag_UnsignedOutputVector = 0x2;
```

If the input or output vector is not marked as unsigned then it should be
interpreted as signed.

It is only valid to mark an input or output vector as unsigned for integral
element types.
...
%result = <32 x i32> call @dx.op.matvecmul.v[32][i32].v[8][i32](
     ...
     0,               ; matrix stride
     0);              ; flags: 0 = signed input & output vectors
```
